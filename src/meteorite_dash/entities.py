...
class Meteorite(Entity):
    def draw(self, surface: pygame.Surface) -> None:
        pygame.draw.circle(surface, METEORITE_COLOR, self.rect.center, self.rect.width // 2)
...
class WaveEnemy(Entity):
    ...
    def _update_vertical(self, dt: float, player_y: int) -> None:
        self._elapsed += dt
        self._y = self._base_y + WAVE_AMPLITUDE * math.sin(
            2 * math.pi * WAVE_FREQUENCY * self._elapsed
        )
...
def spawn_meteorite(rng: random.Random, screen_size: tuple[int, int]) -> Meteorite:
    width, height = screen_size
    diameter = METEORITE_RADIUS * 2
    y = rng.randint(0, height - diameter)
    return Meteorite(pygame.Rect(width, y, diameter, diameter), METEORITE_SPEED)


def spawn_wave_enemy(rng: random.Random, screen_size: tuple[int, int]) -> WaveEnemy:
    width, height = screen_size
    w, h = ENEMY_SIZE
...
def spawn_hunter_enemy(rng: random.Random, screen_size: tuple[int, int]) -> HunterEnemy:
    width, height = screen_size
    w, h = ENEMY_SIZE
    y = rng.randint(0, height - h)
