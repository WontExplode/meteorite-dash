...
from meteorite_dash.config import SAVE_DIR_ENV, SAVE_FILENAME
...
def test_toggle_accessory_equips_and_unequips() -> None:
    progress = Progress(owned_accessories={SHIELD.id})
...
def test_toggle_accessory_respects_ship_slots() -> None:
    progress = Progress(owned_accessories={SHIELD.id, MAGNET.id, ARMOR.id})
...
def test_from_dict_ignores_wrong_types_and_unknown_ids() -> None:
    ...
    assert restored.owned_accessories == set()
